...
#             nn.Conv2d(c2, c2, kernel_size=3, stride=s, padding=1, bias=False),
#             nn.BatchNorm2d(c2),
#             nn.ReLU(inplace=True),
#             nn.Conv2d(c2, c2 * self.expansion, kernel_size=1, stride=1, bias=False),
#             nn.BatchNorm2d(c2 * self.expansion),
#         )
        
#         self.eca = ECA(channels=c2 * self.expansion)
...
#     """Hybrid Attention Module combining CBAM and ECA."""
    
#     def __init__(self, channels, kernel_size=7):
#         super(CBAM_ECA, self).__init__()
#         self.cbam = CBAM(c1=channels, kernel_size=kernel_size)
...
# class ResBlock_CBAM_ECA(nn.Module):
#     def __init__(self, c1, c2, k=1, s=1, expansion=1, downsampling=False):
#         super(ResBlock_CBAM_ECA, self).__init__()
#         self.expansion = expansion
#         self.downsampling = downsampling
...
#         self.attention = CBAM_ECA(channels=c2 * self.expansion)
...
#     def forward(self, x):
#         avg_pool = torch.mean(x, dim=1, keepdim=True)  # Average pooling across channel
#         max_pool = torch.max(x, dim=1, keepdim=True)[0]  # Max pooling across channel
#         return torch.cat([avg_pool, max_pool], dim=1)  # Concatenate pooled features
...
# # Multi-Scale Channel Attention with Z-Pool and Swish
# class ChannelAttention(nn.Module):
#     def __init__(self, channels: int, reduction=16):
#         super(ChannelAttention, self).__init__()
#         self.z_pool = ZPool()  # Z-Pooling for attention
#         self.fc1 = nn.Conv2d(2, channels // reduction, 1, bias=False)  # Reduce dimensionality
#         self.fc2 = nn.Conv2d(channels // reduction, channels, 1, bias=False)  # Back to original channels
#         self.act = Swish()  # Swish activation for smoother learning

#     def forward(self, x: torch.Tensor) -> torch.Tensor:
#         z_pooled = self.z_pool(x)  # Apply Z-Pool
#         attention = self.fc1(z_pooled)  # 1st Conv Layer (Reduction)
#         attention = self.act(attention)  # Swish Activation
#         attention = self.fc2(attention)  # 2nd Conv Layer (Expansion)
#         return x * attention.sigmoid()  # Apply attention and Sigmoid

# # Dynamic Spatial Attention with Multiple Kernel Sizes and Swish
# class SpatialAttention(nn.Module):
...
#         self.conv = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)  # 1D convolution for channel attention
#         self.sigmoid = nn.Sigmoid()
...
#         y = self.conv(y)  # [B, 1, C]
#         y = self.sigmoid(y).permute(0, 2, 1).unsqueeze(-1)  # [B, C, 1, 1]
#         return x * y.expand_as(x)  # Apply ECA attention to input features

# # CBAM Module incorporating Channel, Spatial, and ECA Attention
# class CBAM_ECA(nn.Module):
#     def __init__(self, channels, kernel_size=7):
#         super(CBAM_ECA, self).__init__()
#         self.channel_attention = ChannelAttention(channels)
#         self.spatial_attention = SpatialAttention([3, 7])  # Dynamic kernel sizes 3 and 7
#         self.eca = ECA(channels)  # ECA for efficient channel attention

#     def forward(self, x: torch.Tensor) -> torch.Tensor:
#         x = self.channel_attention(x)  # Apply Channel Attention with Z-Pool and Swish
#         x = self.spatial_attention(x)  # Apply Dynamic Spatial Attention
#         x = self.eca(x)  # Apply ECA Attention
#         return x

# # Updated ResBlock incorporating the Novel CBAM + ECA
# class ResBlock_CBAM_ECA2(nn.Module):
#     def __init__(self, c1, c2, k=1, s=1, expansion=1, downsampling=False):
#         super(ResBlock_CBAM_ECA2, self).__init__()
#         self.expansion = expansion
#         self.downsampling = downsampling
...
#         self.attention = CBAM_ECA(c2 * self.expansion)  # Use the new CBAM_ECA attention module
...
#     def forward(self, x):
#         residual = x
#         out = self.bottleneck(x)  # Pass through the bottleneck
#         out = self.attention(out)  # Apply attention
#         if self.downsampling:
#             residual = self.downsample(x)  # Adjust dimensions for the residual connection
